        ...
        const loadInvoice = async () => {
            if (existingId) {
                ...
                try {
                    ...
                    const invNum = existingNumber || `INV-GRD-${Date.now().toString().slice(-8)}`;
                    setInvoiceNumber(invNum);
                } catch (e) {
                    console.error("Failed to parse invoice data", e);
                }
            } else {
                // Fallback for new invoice without data (shouldn't happen in normal flow but safe to have)
                const invNum = existingNumber || `INV-GRD-${Date.now().toString().slice(-8)}`;
                setInvoiceNumber(invNum);
            }
        ...
        try {
            ...
            const invoiceData = {
                ...
                details: {
                    ...
                    otherItems: data.otherItems || [], // Save other items to DB
                }
            };
            ...
            // Don't show error toast, continue with WhatsApp
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="min-h-screen bg-slate-100 p-4 md:p-8 print:bg-white print:p-0 print:m-0">
            ...
            <div className="max-w-3xl mx-auto mb-6 flex flex-col md:flex-row justify-between items-center gap-4 print:hidden">
                <Button variant="outline" onClick={() => {
                    if (window.history.length > 2) {
                        ...
                        // Opened in new tab or direct link
                        if (window.opener) {
                            window.close();
                        } else {
            ...
            <div className="max-w-3xl mx-auto bg-white p-8 md:p-12 rounded-3xl shadow-sm border border-slate-200 print:shadow-none print:border-0 print:w-full print:max-w-none print:rounded-none relative">
                ...
                <div className="mb-8 p-6 bg-slate-50 rounded-2xl print:bg-transparent print:p-0 print:mb-6">
                    ...
                    <p className="text-sm text-slate-500">{data.customerInfo?.address || "Customer Umum"}</p>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {data.windows?.map((w: any, idx: number) => (
                                <tr key={idx}>
                                    ...
                                    <td className="py-4">
                                        <p className="font-bold text-slate-900">Paket Gorden + Pipa (Jendela {idx + 1})</p>
                                    ...
                                    <td className="py-4 text-sm text-right font-bold text-slate-900">
                                        Rp {(Math.max(1, (Number(w.width) || 0) * (Number(w.height) || 0)) * (data.unitPrice || 0)).toLocaleString("id-ID")}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                            ...
                            {data.otherItems && data.otherItems.length > 0 && (
                                <>
                                    <tr className="bg-slate-50/50">
                                        <td colSpan={4} className="py-2 px-2 text-xs font-bold text-slate-400 uppercase tracking-wider">Produk Lainnya</td>
                                    </tr>
                                    {data.otherItems.map((item: any, idx: number) => (
                                        <tr key={`other-${idx}`}>
                                            <td className="py-4 text-sm font-medium text-slate-400">{(data.windows?.length || 0) + idx + 1}</td>
                                            <td className="py-4">
                ...
                <div className="text-center pt-8 border-t border-slate-100">
                    ...
                    <p className="text-xs text-slate-500">
                        Telah mempercayakan dekorasi rumah Anda kepada {isMounted ? appName : "Sewlovely Homeset"}.
                    </p>
                </div>
            </div>
        </div>
