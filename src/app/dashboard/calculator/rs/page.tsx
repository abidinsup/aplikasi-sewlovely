...
import { ArrowLeft, Calendar, Send, CheckCircle2, Stethoscope } from "lucide-react";
...
export default function SurveyRequestPage() {
    ...
    }, [surveyDate, getAvailableTimeSlots]);
            ...
            <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm backdrop-blur-md bg-white/90">
                <div className="max-w-3xl mx-auto px-4 md:px-8 h-14 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-lg font-bold text-slate-900 tracking-tight">Ajukan Survey Gorden RS/Klinik</h1>
                    </div>
                </div>
            </header>
                ...
                <div className="space-y-6">
                    <section className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                            <div className="p-2 bg-rose-100 rounded-lg text-rose-600"><Stethoscope className="h-5 w-5" /></div>
                            <h2 className="font-bold text-slate-900 text-lg">Data Fasilitas Kesehatan</h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider pl-1">Nama RS/Klinik/PIC</label>
                                <Input
                                    placeholder="Nama RS/Klinik atau PIC"
                                    className="h-12 bg-slate-50 border-slate-200 focus-visible:ring-rose-500 rounded-xl placeholder:text-slate-300"
                            ...
                            <div className="space-y-2 md:col-span-2">
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider pl-1">Alamat Fasilitas</label>
                                <Input
                    ...
                    <section className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                            <div className="p-2 bg-rose-100 rounded-lg text-rose-600"><Calendar className="h-5 w-5" /></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <Input
                                    type="date"
                                    className="h-12 bg-slate-50 border-slate-200 focus-visible:ring-rose-500 rounded-xl cursor-pointer"
                                    value={surveyDate}
                            ...
                            <div className="space-y-2">
                                ...
                                {getAvailableTimeSlots().length === 0 && surveyDate === new Date().toISOString().split('T')[0] ? (
                                    ...
                                    <select
                                        className="h-12 w-full bg-slate-50 border border-slate-200 focus:ring-rose-500 rounded-xl px-4 text-slate-700"
                                        value={surveyTime}
                    ...
                    <Button
                        ...
                        className="w-full h-14 bg-rose-600 hover:bg-rose-500 text-white rounded-2xl font-bold text-lg shadow-xl shadow-rose-600/30 hover:shadow-2xl hover:shadow-rose-600/50 flex items-center justify-center gap-2 transition-all duration-300 hover:scale-[1.01] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isSubmitting ? (
                            <>Mengajukan...</>
                        ) : (
