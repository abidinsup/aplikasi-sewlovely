...
function InvoiceContent() {
    ...
    React.useEffect(() => {
        ...
        const invNum = existingNumber || `INV-SP-${Date.now().toString().slice(-8)}`;
    ...
    const saveInvoiceToSupabase = async () => {
        ...
        try {
            ...
            const invoiceData = {
                ...
                details: {
                    selectedSprei: data.selectedSprei,
                    selectedBedcover: data.selectedBedcover,
        ...
        <div className="min-h-screen bg-slate-100 p-4 md:p-8 print:bg-white print:p-0 print:m-0">
            ...
            <div className="max-w-3xl mx-auto bg-white p-8 md:p-12 rounded-3xl shadow-sm border border-slate-200 print:shadow-none print:border-0 print:w-full print:max-w-none print:rounded-none relative">
                ...
                <div className="mb-8 p-6 bg-slate-50 rounded-2xl print:bg-transparent print:p-0 print:mb-6">
                    ...
                    <p className="font-bold text-slate-900 text-lg">{data.customerInfo?.name || "Pelanggan"}</p>
                    <p className="text-sm text-slate-500">{data.customerInfo?.address || "-"}</p>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b-2 border-slate-100">
                                ...
                                <th className="py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Ukuran</th>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {data.selectedSprei && (
                                <tr>
                                    <td className="py-4 text-sm font-medium text-slate-400">1</td>
                                    <td className="py-4">
                                        <p className="font-bold text-slate-900">Sprei Premium {data.selectedSprei.label}</p>
                                        <div className="text-xs text-slate-500 mt-1">Bahan: Cotton Premium</div>
                                    </td>
                                    <td className="py-4 text-sm text-center font-medium text-slate-700">
                                        {data.selectedSprei.size}
                                    </td>
                                    <td className="py-4 text-sm text-right font-bold text-slate-900">
                                        Rp {data.selectedSprei.price.toLocaleString("id-ID")}
                                    </td>
                                </tr>
                            )}
                            {data.selectedBedcover && (
                                <tr>
                                    <td className="py-4 text-sm font-medium text-slate-400">{data.selectedSprei ? 2 : 1}</td>
                                    <td className="py-4">
                                        <p className="font-bold text-slate-900">Bedcover {data.selectedBedcover.label}</p>
                                        <div className="text-xs text-slate-500 mt-1">Bahan: Silikon Lembut</div>
                                    </td>
                                    <td className="py-4 text-sm text-center font-medium text-slate-700">
                                        {data.selectedBedcover.size}
                                    </td>
                                    <td className="py-4 text-sm text-right font-bold text-slate-900">
                                        Rp {data.selectedBedcover.price.toLocaleString("id-ID")}
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                <div className="text-center pt-8 border-t border-slate-100">
                    ...
                    <p className="text-xs text-slate-500">Kenyamanan tidur Anda adalah prioritas kami.</p>
                </div>
            </div>
        </div>
    );
...
export default function SpreiInvoicePage() {
    return (
